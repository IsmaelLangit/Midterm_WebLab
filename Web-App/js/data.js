var block = [

	// 6th floor
		{
		"category" : "LiR", 
		"min" : "690", 
		"max" : "700",
		"mid" : "348, 234",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "HGR", 
		"min" : "912", 
		"max" : "994",
		"mid" :"340, 234",
         "color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "973",
		"max" : "998",
		"mid" :"318, 232",
		"color" : "#4F82B7", //blue
		 "floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "956", 
		"max" : "973",
		"mid" :"308, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "943", 
		"max" : "956",
		"mid" :"284, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "940", 
		"max" : "943",
		"mid" :"274, 232",
		 "color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "930", 
		"max" : "940",
		"mid" :"250, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "910", 
		"max" : "930",
		 "mid" :"240, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Hg", 
		"min" : "900", 
		"max" : "910",
		"mid" :"216, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" :  "843", 
		"max" : "897",
		"mid" :"206, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "823", 
		"max" : "843",
		"mid" :"184, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "823", 
		"max" :   "824",
		"mid" :"174, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "821", 
		"max" : "823",
		 "mid" :"152, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "818", 
		"max" : "821",
		"mid" :"142, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "811", 
		"max" : "818",
		"mid" :"122, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "808", 
		"max" : "811",
		"mid" :"112, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "808", 
		"max" :  "809",
		"mid" :"92, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Li", 
		"min" : "808", 
		"max" :  "809",
		"mid" :"82, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "HG-PR", 
		"min" : "973", 
		"max" : "974",
		"mid" :"63, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "HG-PR", 
		"min" : "910", 
		"max" : "973",
		"mid" :"53, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
        {
		"category" : "Li-Pr", 
		"min" : "809", 
		"max" : "910",
		"mid" :"33, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
        {
		"category" : "Ar-Pr", 
		"min" : "746", 
		"max" : "809",
		"mid" :"23, 232",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		{
		"category" : "AS", 
		"min" : "600", 
		"max" : "610",
		"mid" : "783, 234",
		"color" : "#81649C", //violet
		"floor" : "6"
							
		},
		{
		"category" : "AS", 
		"min" : "610", 
		"max" : "611",
		"mid" : "793, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "610", 
		"max" : "611",
		"mid" : "818, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "610", 
		"max" : "611",
		"mid" : "828, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "610", 
		"max" : "611",
		"mid" : "854, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "610", 
		"max" : "611",
		"mid" : "864, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "611", 
		"max" : "612",
		"mid" : "888, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "612", 
		"max" : "613",
		"mid" : "898, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "612", 
		"max" : "613",
		"mid" : "925, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "612", 
		"max" : "613",
		"mid" : "934, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS",
		"min" : "613", 
		"max" : "614",
		"mid" : "959, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "613", 
		"max" : "615",
		"mid" : "969, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "615", 
		"max" : "616",
		"mid" : "996, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "615", 
		"max" : "616",
		"mid" : "1005, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "615", 
		"max" : "616",
		"mid" : "1030, 232",
		"color" : "#81649C", //violet
		"floor" : "6"

		},
		{
		"category" : "AS", 
		"min" : "615", 
		"max" : "616",
		"mid" : "1040, 232",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},

		{
		"category" : "ASR", 
		"min" : "620", 
		"max" : "621",
		"mid" : "394, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "ASR", 
		"min" : "619", 
		"max" : "620",
		"mid" : "385, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "ASR", 
		"min" : "621", 
		"max" : "643",
		"mid" : "366, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "ASR",  
		"min" : "708", 
		"max" : "709", 
		"mid" : "356, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "619", 
		"max" : "620",
		"mid" : "338, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "328, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "311, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "302, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "283, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "273, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "620", 
		"max" : "621",
		"mid" : "255, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "621", 
		"max" : "624",
		"mid" : "245, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "623", 
		"max" : "624",
		"mid" : "227, 140",
		"color" : "#4F82B7",  //blue 
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "624", 
		"max" : "628",
		"mid" : "217, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "628", 
		"max" : "633",
		"mid" : "200, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "633", 
		"max" : "658",
		"mid" : "190, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "657", 
		"max" : "658",
		"mid" : "171, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "657", 
		"max" : "658",
		"mid" : "161, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "658", 
		"max" : "660",
		"mid" : "143, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "660", 
		"max" : "671",
		"mid" : "133, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "671", 
		"max" : "693",
		"mid" : "114, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "693", 
		"max" : "719",
		"mid" : "104, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "720",
		"max" : "725",
		"mid" : "86, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "725", 
		"max" : "780",
		"mid" : "76, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "780", 
		"max" : "796",
		"mid" : "59, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "796", 
		"max" : "799",
		"mid" : "49, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "621", 
		"max" : "628",
		"mid" : "31, 140",
		"color" : "#4F82B7", //blue
		"floor" : "6"
		},
		                
		{
		"category" : "AS", 
		"min" : "628", 
		"max" : "660",
		"mid" : "22, 140",
		"color" : "#81649C", //violet
		"floor" : "6"
		},

		{
	      "category" : "ASR",
	      "min" : "600", 
	      "max" : "611" ,            
	      "mid" : "678, 140",
	      "color" : "#81649C", //violet                  
	      "floor" : "6"
	    },
	    {
	      "category" : "ASR",
	      "min" : "612", 
	      "max" : "618",                  
	      "mid" :"686, 140",      
	      "color" : "#4F82B7", //blue                                            
	      "floor" : "6"   
	    },
	    {
	     "category" : "AS", 
	      "min" : "618",
	      "max" : "619",                      
	      "mid" :"686, 140",                      
	      "color" : "#81649C", //violet                                          
	      "floor" : "6"
	    },                            
	    {
	      "category" : "AS", 
	      "min" : "618",
	      "mid" : "720, 140",                   
	      "color" :"#4F82B7", //blue                 
	      "floor" : "6"
	     },               
	    
	    {
	      "category" : "AS", 
	      "min" : "618",
	      "max" : "619",
	       "mid" : "749, 140",                
	      "color" :"#81649C", //violet                 
	      "floor" : "6"
	    },                
	    {
	      "category" : "AS", 
	      "min" : "617", 
	      "max" : "618",
	      "mid" : "757, 140",                 
	      "color" :"#4F82B7", //blue                
	      "floor" : "6"
	    },               
	    {
	      "category" : "AS", 
	      "min" : "617", 
	      "max" : "618",
	       "mid" : "784, 140",                
	      "color" :"#81649C", //violet                
	      "floor" : "6"
	   },                
	   {
	      "category" : "AS", 
	      "min" : "616",
	      "max" : "617",
	      "mid" : "792, 140",                  
	      "color" :"#4F82B7", //blue                                                
	      "floor" : "6"
	    },             
	    {
	      "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "818, 140",                 
	      "color" :"#81649C", //violet                 
	      "floor" : "6"
	    },                
	    {
	      "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "826, 140",                   
	      "color" :"#4F82B7", //blue                
	      "floor" : "6"
	    },                
	    {
	      "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "854, 140",                   
	      "color" :"#81649C", //violet                  
	      "floor" : "6"
	    },               
	    {
	      "category" : "AS", 
	      "min" : "616",  "max" : "617", 
	       "mid" : "864, 140",                
	      "color" :"#4F82B7", //blue                 
	      "floor" : "6"
	     },                 
	     {
	       "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "888, 140",                  
	      "color" :"#81649C", //violet            
	      "floor" : "6"
	    },               
	    {
	      "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "897, 140",                 
	      "color" :"#4F82B7", //blue                 
	      "floor" : "6"
	    },               
	    {
	      "category" : "AS", 
	      "min" : "616",  "max" : "617", 
	      "mid" : "925, 140",                  
	      "color" :"#81649C", //violet
	      "floor" : "6"
	   },                 
	   {
	      "category" : "AS", 
	      "min" : "616",  
	      "max" : "617", 
	       "mid" : "934, 140",                 
	      "color" :"#4F82B7", //blue                                       
	      "floor" : "6"
	   },              
	   {
	      "category" : "As-Pr", 
	      "min" : "617",  
	      "max" : "618",
	      "mid" : "960, 140",                        
	      "color" :"#81649C", //violet                                                
	      "floor" : "6"
	    },                
	    {
	      "category" : "As-Pr", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "968, 140",                        
	      "color" :"#4F82B7", //blue                                                
	      "floor" : "6"
	    },                   
	    {
	     "category" : "As-Pr", 
	      "min" : "616",  
	      "max" : "617", 
	      "mid" : "996, 140",                        
	      "color" :"#81649C", //violet                                                
	      "floor" : "6"
	     },                 
	    {
	    "category" : "As-Pr", 
	    "min" : "616",  "max" : "617", 
	    "mid" : "1006, 140",                   
	    "color" :"#4F82B7", //blue                 
	    "floor" : "6"
	    },   

	    {
	     "category" : "As-Pr", 
	      "min" : "614",  
	      "max" : "616", 
	      "mid" : "1032, 140",               
	      "color" :"#81649C", //violet                 
	      "floor" : "6"
	   },               
	   {
	      "category" : "As-Pr", 
	      "min" : "610",  
	      "max" : "613", 
	      "mid" : "1040, 140",                  
	      "color" :"#4F82B7", //blue                
	      "floor" : "6"
	    },

	    //5th floor
    {
		"category" : "SS",
		"min" : "373",
		"max" : "375",
        	"mid" : "137, 143",
		"color" : "#81649C", //violet
       		 "floor" : "5"
	},
	{
		"category" : "SS",
		"min" : "370",
		"max" : "375",
		"mid" : "146, 143",
		"color" : "#4F82B7",
      		"floor" : "5"
	},
	{
        "category" : "SS", 
        "min" : "360",
        "max" : "375",
        "mid" : "170,143",
        "color" : "#81649C",
        "floor" : "5"
    },
    {
          "category" : "SS",
          "min" : "378",
          "max" : "389",
          "mid" : "179,143",
          "color" : "#4F82B7",
          "floor" : "5"
    },
    {
            "category" : "SS",
            "min" : "372",
            "max" : "389" ,
            "mid" : "201, 143",
            "color" : "#81649C", //violet
            "floor" : "5"
        
        
    },
    {
        "category": "SS",
        "min" : "372",
        "max" : "373",
        "mid" : "210, 143",
        "color" : "#4F82B7", //blue
        "floor" : "5"
    },
    {
        "category" : "SS",
        "min" : "371",
        "max" : "373",
        "mid" : "235, 143",
        "color" : "#81649C" ,//violet
		"floor" : "5"
        
    },
    {  
        "category" : "SS",
        "min" : "371",
        "max" : "372",
        "mid" : "244, 143",
        "color" :"#4F82B7", //blue
        "floor" : "5"
        
    },
    {
        "category" : "SS" ,
        "min" : "371",
        "max" : "372",
        "mid" : "269, 143",
        "color" :"#81649C", //violet
        "floor" : "5"
        
    },
    {
        "category" : "SS" ,
        "min" : "370",
        "max" : "371",
		"mid" : "278, 143",
		"color" :"#4F82B7", //blue
        "floor" : "5"
       
    },
	{
          
        "category" : "SS",
        "min" : "364",
        "max" : "371",
        "mid" : "303, 143",
        "color" :"#81649C", //violet
        "floor" : "5"
				
    },
    
    {
        "category" : "SS" ,
        "min" : "362",
        "max" : "363",
        "mid" : "312, 143",
        "color" :"#4F82B7", //blue
        "floor" : 5
    },
    {
        "category" : "SS",
        "min" : "361",
        "max" : "363",
		"mid" : "336, 143",
        "color" :"#81649C",               
		"floor" : "5"
    },
    
    {
    
        "category" : "SS-PR",
        "min" : "333",
        "max" : "353",
        "mid" : "345, 143",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
    {
        "category" : "SS-PR",
        "min" : "320",
        "max" : "331",
		"mid" : "367, 143",
		"color" :"#81649C",
        "floor" : "5"
    },
    
    {
        "category" : "SS-PR",
        "min" : "300",
        "max" : "316",
		"mid" : "376, 143",
        "color" :"#4F82B7", //blue
        "floor" : "5"
    },
    
    {
        "category" : "SSG-PR" ,
        "min" : "340",
        "max" : "379",
        "mid" : "671, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "SS-PR",
        "min" : "325",
        "max" : "341",
        "mid" : "681, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"
    },
    
	{
        "category" : "SSG-PR",
        "min" : "340",
        "max" : "349",
        "mid" : "701, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "SS",
        "min" : "512",
        "max" : "739",
        "mid" : "710, 141",
        "color" :"#4F82B7",//blue
        "floor" : "5"
    },
    
    {
        "category" : "SS" ,
        "min" : "280",
        "max" : "513",
        "mid" : "731, 141",
        "color" :"#81649C",//violet
        "floor" : "5"
    },
    
    {
        "category" : "SS",
        "min" : "55",
        "max" : "281",
        "mid" : "740, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"
    },
    {
        "category" : "SS" ,
        "min" : "1",
        "max" : "510",
        "mid" : "763, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "SS",
        "min" : "347",
        "max" : "350",
        "mid" : "772, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"
    },
    
	{
        "category" : "SS",
        "min" : "346",
        "max" : "437",
		"mid" : "793, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
	{
        "category" : "SS",
        "min" : "345", 
        "max" : "347",
        "mid" : "802, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    {
        "category" : "SS",
        "min" : "343",
        "max" : "346",
        "mid" : "821, 141", 
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "F",
        "min" : "53",
        "max" : "1032",
        "mid" : "830, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
    {
        "category" : "SS/G",
        "min" : "340",
        "max" : "343",
        "mid" : "850, 141",
	    "color" :"#81649C", //violet
        "floor" : "5"
    },
    
	{
        "category" : "SS/M",
        "min" : "338",
        "max" : "339",
        "mid" : "859, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
   {
       "category" : "SS/B-D",
        "min" : "330",
        "max" : "337",
        "mid" : "880, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    
    },
    
    {
        "category" : "SS/C",
        "min" : "328",
        "max" : "333",
        "mid" : "889, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },

    {
        "category" : "SS",
        "min" : "323",
        "max" : "329",
        "mid" : "911, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "SS/F",
        "min" : "328",
        "max" : "331",
        "mid" : "920, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },

    {
        "category" : "SS",
        "min" : "320",
        "max" : "324",
        "mid" : "939, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "SS Ei",
        "min" : "305",
        "max" : "307",
        "mid" : "948, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
    {
        "category" : "SS/T",
        "min" : "301",
        "max" : "303",
        "mid" : "968, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "Re" ,
        "min" : "302",
        "max" : "306",
        "mid" : "977, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
    {
        "category" : "Re",
        "min" : "300",
        "max" : "301",
		"mid" : "995, 141",
		"color" :"#81649C", //violet
        "floor" : "5"
    },
    
	{
        "category" : "Re",
        "min" : "282",
        "max" : "300",
        "mid" : "1004, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    
	{
        "category" : "Re" ,
        "min" : "268",
        "max" : "271",
        "mid" : "1023, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    },
    
    {
        "category" : "Re",
        "min" : "261",
        "max" : "267",
        "mid" : "1032, 141",
        "color" :"#4F82B7", //blue
        "floor" : "5"    
    }, 
    
	{
        "category" : "Re",
        "min" : "253",
        "max" : "262",
        "mid" : "104, 141",
        "color" :"#81649C", //violet
        "floor" : "5"
    
    },
    
	{
        "category" : "Re" ,
        "min" : "242",
        "max" : "249",
        "mid" : "1057, 141",
		"color" :"#4F82B7", //blue
        "floor" : "5"    
    },
    

	    //4th floor

	{
		"category" : "FR",
		"min" : "305",
		"max" : "960",
		"mid" : "157.5, 228, 174.5, 228",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "FR",
		"min" : "378",
		"max" : "900",
		"mid" : "157.5, 252, 174.5, 252",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "359",
		"max" : "960",
		"mid" : "157.5, 262, 174.5, 262",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "FR",
		"min" : "323",
		"max" : "960",
		"mid" : "157.5, 279, 174.5, 279",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "FR",
		"min" : "959",
		"max" : "960",
		"mid" : "157.5, 289, 174.5, 289",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "759",
		"max" : "960",
		"mid" : "157.5, 309, 174.5, 309",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "499",
		"max" : "760",
		"mid" : "157.5, 319, 174.5, 319",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "315",
		"max" : "500",
		"mid" : "157, 334, 174.5, 334",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "1",
		"max" : "316",
		"mid" : "157.5, 342, 174.5, 342",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "551",
		"max" : "900",
		"mid" : "159, 296, 128, 296",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "551",
		"max" : "900",
		"mid" : "161, 305, 130, 305",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "899",
		"max" : "960",
		"mid" : "161, 322, 130, 322",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "899",
		"max" : "960",
		"mid" : "161, 331, 130, 331",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "959",
		"max" : "960",
		"mid" : "157.5, 280.8, 8 ,280.8",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "959",
		"max" : "960",
		"mid" : "161, 343.5, 7, 343.5",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "GN",
		"min" : "688",
		"max" : "9685",
		"mid" : "292, 140.5",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "GN",
		"min" : "658",
		"max" : "1635",
		"mid" : "284, 140.5",
		"color" : "#81649C",
		"floor" : "4"
	},
	{
		"category" : "F",
		"min" : "599",
		"max" : "9941",
		"mid" : "261, 140.5",
		"color" : "#4F82B7",
		"floor" : "4"
	},
	{
		"category" : "GN",
		"min" : "223",
		"max" : "913",
		"mid" : "252, 140.5",
		"color" : "#81649C",
		"floor" : "4"
	}

];



var range = [
	{
		"category" : "AS",
			"min" : "600", 
			"max" : "796"
	},
	{
		"category" : "As-Pr",
			"min" : "610", 
			"max" : "618" 
	},
	{
		"category" : "ASR",
			"min" : "610", 
			"max" : "618" 
	},
	{
		"category" : "ArR",
            "min" : "708", 
			"max" : "709" 
	},
	{
		"category" : "F",
			"min" : "001", 
			"max" : "959" 
	},
	{
		"category" : "FIC",
			"min" : "233", 
			"max" : "9940" 
	},
	{
		"category" : "G",
		"min" : "20", 
		"max" : "393" 

	},
	{
		"category" : "GN",
		"min" : "658", 
		"max" : "9684" 

	},
	{
		"category" : "GR",
		"min" : "1998", 
		"max" : "1998" 

	},
	{
		"category" : "Hg",
		"min" : "900", 
		"max" : "998" 

	},
	{
		"category" : "HgR",
		"min" : "903", 
		"max" : "912" 

	},
	{
		"category" : "La",
		"min" : "410", 
		"max" : "495" 

	},
	{
		"category" : "La-PR",
		"min" : "410", 
		"max" : "416" 

	},
	{
		"category" : "Li",
		"min" : "808", 
		"max" : "897" 

	},
	{
		"category" : "LiR",
		"min" : "803", 
		"max" : "860" 

	},
	{
		"category" : "PP",
		"min" : "100", 
		"max" : "399" 

	},
	{
		"category" : "PS",
		"min" : "505", 
		"max" : "599" 

	},
	{
		"category" : "PS-PR",
		"min" : "543", 
		"max" : "595" 

	},
	{
		"category" : "PSR",
		"min" : "500", 
		"max" : "599" 

	},
	{
		"category" : "Re",
		"min" : "210", 
		"max" : "305" 

	},
	{
		"category" : "SS",
		"min" : "053", 
		"max" : "738" 

	},
	{
		"category" : "SS Ei",
		"min" : "305", 
		"max" : "307" 

	},
	{
		"category" : "SS/B-D",
		"min" : "330", 
		"max" : "336" 	

	},
	{
		"category" : "SS/C",
		"min" : "328", 
		"max" : "332" 

	},
	{
		"category" : "SS/F",
		"min" : "53", 
		"max" : "1031" 

	},
	{
		"category" : "SS/G",
		"min" : "340", 
		"max" : "342" 

	},
	{
		"category" : "SS/M",
		"min" : "338", 
		"max" : "338" 

	},
	{
		"category" : "SS/T",
		"min" : "301", 
		"max" : "302" 					
	},
	{
		"category" : "SSG-PR",
		"min" : "340", 
		"max" : "378" 

	},
	{
		"category" : "SS-PR",
		"min" : "300", 
		"max" : "352" 
	} 
];
	function myFunction() {
		var myObj1, myJSON1, text1, objBlock, myObj2, myJSON2, text2, objRange;
		var inputCat = document.getElementById("mySelect").value;
		var element = document.getElementById("text-search");
		var num =document.getElementById("text-search").value;

		//Storing data (block):
		myJSON1 = JSON.stringify(block);
		localStorage.setItem("data1", myJSON1);

		//Retrieving data(block):
		text1 = localStorage.getItem("data1");
		objBlock = JSON.parse(text1);

		//Storing data (range):
		myJSON2 = JSON.stringify(range);
		localStorage.setItem("data2", myJSON2);

		//Retrieving data(range):
		text2 = localStorage.getItem("data2");
		objRange = JSON.parse(text2);

		//Setting min and max attribute
		for (let i = 0; i <= objBlock.length ; i++ ){
			let cat = objRange[i].category;
			var minRange = objRange[i].min;
			var maxRange = objRange[i].max;

			if (inputCat == cat){
				element.setAttribute("min", minRange);
				element.setAttribute("max", maxRange);
			}
		}

		//Find the index of input of the user from the json file - range
		for (let i = 0; i <= objBlock.length ; i++ ){
			let cat = objBlock[i].category;
			var mincat = objBlock[i].min;
			var maxcat = objBlock[i].max;

			if (inputCat == cat && num >= mincat && num <= maxcat ){
				document.getElementById("output").innerHTML = "You've search for : " + category + "-" + num;
				localStorage.setItem("index", i);    
			}
		}
	}
